...
export class InvoiceController {
    ...
    @ApiResponse({
        status: 200,
        description: 'Success',
        type: InvoiceModel.GetAllInvoice,
    })
    async getAll(
        @Query() query: InvoiceModel.IInvoiceQueryParams,
        @Req() req: Request,
        @Res() res: Response,
    ): Promise<any> {
        try {
    ...
    @ApiResponse({
        status: 200,
        description: 'Success',
        type: InvoiceModel.GetByIdInvoice,
    })
    async getById(
    ...
    async create(
        @Body() body: InvoiceModel.CreateInvoice,
        @Req() req: Request,
        @Res() res: Response,
    ): Promise<any> {
        try {
    ...
    async update(
        @Body() body: InvoiceModel.UpdateInvoice,
        @Req() req: Request,
        @Res() res: Response,
    ): Promise<any> {
        try {
    ...
    async sendMessageBatch(
        @Body() body: InvoiceModel.SendMessageBatch,
        @Req() req: Request,
        @Res() res: Response,
    ): Promise<any> {
        try {
            const data = await this._invoiceService.sendMessageBatch(req, body.data);
            return res.status(HttpStatus.OK).json(data);
        } catch (error) {
            console.log('error =>', error);
            const status = error.status || HttpStatus.INTERNAL_SERVER_ERROR;
