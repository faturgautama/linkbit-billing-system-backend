  ...
  async sendMessageBatch(req: Request, data: any[]): Promise<any> {
    try {
      ...
      for (const item of data) {
        ...
        const checkLog =
          await this._prismaService.log_whatsapp_message.findFirst({
            where: {
        ...
        const isSameDay =
          new Date(checkLog.sent_at).toDateString() ===
        ...
        if (!isSameDay) {
          ...
          req['user']['id_setting_company'] =
            invoice.pelanggan.id_setting_company;

          const resultMessage =
            await this._channelProviderRouterService.handleSendMessage(
              req,
              'INVOICE',
              invoice,
            );
