...
export class LaporanService {
  ...
  async getRekapTagihanBulanan(
    req: Request,
    query: LaporanModel.IQueryParamLaporanTagihan,
  ): Promise<LaporanModel.GetRekapTagihanBulanan> {
    try {
      if (!query.date) {
        return {
          status: false,
      ...
      const start_date = format(
        startOfMonth(new Date(query.date)),
        'yyyy-MM-dd',
      );
      ...
      const invoice = await this._prismaService.invoice.findMany({
        where: queries,
        include: {
          pelanggan: {
            include: {
  ...
  async getDetailTagihanBulanan(
    req: Request,
    query: LaporanModel.IQueryParamLaporanTagihan,
  ): Promise<LaporanModel.GetDetailTagihanBulanan> {
    try {
      if (!query.date) {
        return {
          status: false,
      ...
      let queries: any = {
        ...
        pelanggan: {
          id_setting_company: parseInt(
            req['user']['id_setting_company'] as any,
          ),
        },
      };
  ...
  async getRekapPembayaranBulanan(
    req: Request,
    query: LaporanModel.IQueryParamLaporanPembayaran,
  ): Promise<any> {
    try {
      if (!query.date) {
        return {
      ...
      const payment = await this._prismaService.payment.findMany({
        where: queries,
        include: {
          pelanggan: true,
        },
  ...
  async getDetailPembayaranBulanan(
    req: Request,
    query: LaporanModel.IQueryParamLaporanPembayaran,
  ): Promise<LaporanModel.GetDetailPembayaranBulanan> {
    try {
      if (!query.date) {
        return {
          status: false,
    ...
    try {
      ...
      const report = pelangganList.map((pelanggan, index) => {
        ...
        for (let month = 1; month <= 12; month++) {
          ...
          const jumlahBayar =
            monthPayments.reduce((sum, p) => sum + p.payment_amount, 0) || null;
  ...
  async getTagihanKsoMitra(req: Request, periode: string) {
    try {
      ...
      const mitra = await this._prismaService.setting_company.findMany({
        where: {
          is_cabang: false,
          is_mitra: true,
          is_active: true,
      ...
      for (let item of mitra) {
        ...
        payload.jumlah_pelanggan = await this._prismaService.pelanggan.count({
          where: {
            id_setting_company: item.id_setting_company,
          },
        });

        const invoice = await this._prismaService.payment.findMany({
          where: {
            pelanggan: {
              id_setting_company: item.id_setting_company,
            },
        ...
        payload.total_tagihan =
          payload.jumlah_pemasukan +
          payload.total_bhp_uso +
          payload.total_pph_final +
          payload.total_kso;
        payload.status_bayar = 'PENDING';

        const getTagihanKso = await this._prismaService.tagihan_kso.findFirst({
          where: {
            id_setting_company: item.id_setting_company,
            periode: {
              gte: `${start_date}T00:00:00.000Z`,
        ...
        if (!getTagihanKso) {
          let createTagihanKso = await this._prismaService.tagihan_kso.create({
            data: {
              ...payload,
              no_tagihan_kso: this._utilityService.onFormatDate(
                new Date(),
                'DD/MM/yyyy HH:mm',
              ),
              create_at: new Date(),
          ...
          if (getTagihanKso.status_bayar == 'PENDING') {
            let updateTagihanKso = await this._prismaService.tagihan_kso.update(
              {
                where: {
      ...
      const tagihan_kso = await this._prismaService.tagihan_kso.findMany({
        where: {
          periode: {
            gte: `${start_date}T00:00:00.000Z`,
            lte: `${end_date}T23:59:59.999Z`,
      ...
      return {
        ...
        data: tagihan_kso.map((item) => {
          return {
            ...
            periode: this._utilityService.onFormatDate(
  ...
  async updateTagihanKsoMitra(
    req: Request,
    id_tagihan_kso: string,
  ): Promise<any> {
    try {
      const tagihanKso = await this._prismaService.tagihan_kso.findFirst({
      ...
      let res = await this._prismaService.tagihan_kso.update({
        where: { id_tagihan_kso: parseInt(id_tagihan_kso as any) },
        data: {
          status_bayar:
            tagihanKso.status_bayar == 'PENDING' ? 'PAID' : 'PENDING',
          update_at: new Date(),
