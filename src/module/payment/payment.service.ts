...
export class PaymentService {
  ...
  async getAll(
    req: Request,
    query: PaymentModel.IPaymentQueryParams,
  ): Promise<PaymentModel.GetAllPayment> {
    try {
      // Bersihkan query dari nilai kosong
      ...
      const newQueries: any = {
        ...
        ...(queries.payment_status && {
          payment_status: {
            contains: queries.payment_status,
            mode: 'insensitive',
          },
        }),
        ...(queries.payment_method && {
          payment_method: {
            contains: queries.payment_method,
            mode: 'insensitive',
          },
        }),
        ...(queries.payment_number && {
          payment_number: {
            contains: queries.payment_number,
            mode: 'insensitive',
          },
        }),

        // Filter payment_date range / exact
        ...(queries.payment_date_start && queries.payment_date_end
          ? {
              payment_date: {
                gte: new Date(
                  new Date(queries.payment_date_start).setHours(0, 0, 0, 0),
                ),
                lte: new Date(
                  new Date(queries.payment_date_end).setHours(23, 59, 59, 999),
                ),
              },
            }
          : queries.payment_date_start
          ? {
              payment_date: {
                gte: new Date(
                  new Date(queries.payment_date_start).setHours(0, 0, 0, 0),
        ...
        invoice: {
          ...(queries.invoice_number && {
            invoice_number: {
              contains: queries.invoice_number,
              mode: 'insensitive',
            },
          }),
        ...
        pelanggan: {
          ...
          ...(queries.pelanggan_code && {
            pelanggan_code: {
              contains: queries.pelanggan_code,
              mode: 'insensitive',
            },
          }),
        },
      ...
      if (queries.search) {
        ...
        newQueries.OR = [
          ...
          {
            pelanggan: {
              pelanggan_code: { contains: keyword, mode: 'insensitive' },
            },
          },
      ...
      if (newQueries.invoice && Object.keys(newQueries.invoice).length === 0)
        delete newQueries.invoice;
      if (
        newQueries.pelanggan &&
        Object.keys(newQueries.pelanggan).length === 0
      )
        delete newQueries.pelanggan;
      ...
      return {
        ...
        data: res.map((item) => ({
          ...
          payment_date:
            item.payment_status == 'PENDING' ? null : item.payment_date,
  ...
  async getById(id_payment: number): Promise<any> {
    try {
      let res: any = await this._prismaService.payment.findUnique({
      ...
      return {
        ...
        data: {
          ...
          tagihan_editor_invoice:
            res.pelanggan.setting_company.tagihan_editor_invoice,
          ...
          payment_date:
            res.payment_status == 'PENDING' ? null : res.payment_date,
          ...
          token: this._utilityService.onEncrypt(
            JSON.stringify({ id_invoice: res.id_invoice }),
          ),
        },
      };
    } catch (error) {
  ...
  async getCheckoutPageUrl(id_invoice: number) {
    try {
      ...
      const token = this._utilityService.onEncrypt(
        JSON.stringify({ id_invoice: invoice.data.id_invoice }),
      );
  ...
  async getDataFromTokenCheckout(token: string) {
    try {
      ...
      let invoice = await this._invoiceService.getById(
        parseInt(data.id_invoice ? data.id_invoice : data),
      );

      let checkIsPaymentExist = await this._prismaService.payment.findFirst({
        where: {
          invoice: {
      ...
      const userEntry = await this._prismaService.user.findFirst({
        where: {
      ...
      if (
        checkIsPaymentExist.payment_provider != 'MANUAL' &&
        checkIsPaymentExist.payment_method != 'QRIS'
      ) {
        const checkExpiredXenditPayload = {
          ...
          headers: {
            Authorization: `Basic ${Buffer.from(
              `${invoice.data.api_key_pg}:`,
            ).toString('base64')}`,
          },
        };

        const checkExpiredXendit = await firstValueFrom(
          this._axiosService.onAxiosRequest(checkExpiredXenditPayload),
        );
      ...
      return {
        ...
        data: {
          ...
          payment: {
            ...checkIsPaymentExist,
            user_entry:
              checkIsPaymentExist.create_by == 9999
                ? 'SISTEM'
                : userEntry
                ? userEntry.full_name
                : 'SISTEM',
  ...
  async getPaymentMethodForInternal(
    req: Request,
  ): Promise<PaymentModel.GetAllPayment> {
    try {
      const settingCompany =
        await this._prismaService.setting_company.findUnique({
          where: {
            id_setting_company: parseInt(req['user']['id_setting_company']),
          },
        });
      ...
      const params = {
        ...
        headers: {
          Authorization: `Basic ${Buffer.from(
            `${settingCompany.api_key_pg}:`,
          ).toString('base64')}`,
        },
      };

      return await firstValueFrom(
        this._axiosService.onAxiosRequest(params).pipe(
          map((result) => {
            result.data = result.data.filter((item: any) => {
              if (
                ...
                item.currency == 'IDR' &&
                item.is_activated &&
                item.code.toLowerCase() != 'sahabat_sampoerna'
              ) {
                return item;
              }
            });
  ...
  async getPaymentMethod(token: string): Promise<PaymentModel.GetAllPayment> {
    try {
      ...
      const invoice = await this._invoiceService.getById(
        parseInt(decryptedData),
      );
      ...
      const pelanggan = await this._prismaService.pelanggan.findUnique({
        where: {
      ...
      const xendit_payment_method =
        await this._prismaService.xendit_payment_method.findMany({
          where: {
            is_active: true,
          },
        });
      ...
      let newData: any[] = [
        ...xendit_payment_method.map((item: any) => {
          let admin_fee_after_vat =
            parseFloat(item.payment_method_fee) +
            parseFloat(item.payment_method_fee) *
              parseFloat(process.env.XENDIT_VAT_FEE);

          return {
            payment_method_type: item.payment_method_type,
            payment_method_name: item.payment_method_name,
            payment_method_code: item.payment_method_code,
            payment_method_instruction: this.getPaymentMethodCaraBayar(
              item.payment_method_code,
            ),
            image: this._imageHelperService.getImageUrl(
              item.payment_method_code,
            ),
  ...
  async changePaymentMethod(token: string) {
    try {
      ...
      const checkIsPaymentExist = await this._prismaService.payment.findFirst({
        where: {
      ...
      if (checkIsPaymentExist) {
        if (
          checkIsPaymentExist.payment_method != 'QRIS' &&
          checkIsPaymentExist.payment_provider == 'XENDIT'
        ) {
          ...
          const updateExpiredDateVaXendit = await firstValueFrom(
            this._axiosService.onAxiosRequest(updateVirtualAccountParams),
          );

          if (
            !updateExpiredDateVaXendit.status &&
            updateExpiredDateVaXendit.data.error_code !=
              'CALLBACK_VIRTUAL_ACCOUNT_INACTIVE_ERROR'
          ) {
            return {
              status: false,
        ...
        const updateStatusPayment = await this._prismaService.payment.delete({
          where: {
  ...
  async payment(payload: PaymentModel.CreatePayment): Promise<any> {
    try {
      if (
        payload.payment_method_code == 'BCA' &&
        parseFloat(payload.payment_amount as any) < 10000
      ) {
        return {
          status: false,
      ...
      const decryptedData = this._utilityService.onDecrypt(
      ...
      if (usePgAdminFee) {
        FEE_AMOUNT = {
          VA: process.env.XENDIT_VA_FEE
            ? parseInt(process.env.XENDIT_VA_FEE)
            : 0,
          QR: process.env.XENDIT_QR_FEE
            ? parseFloat(process.env.XENDIT_QR_FEE)
            : 0,
        };

        admin_fee =
          payload.payment_method_code == 'QRIS'
            ? parseFloat(payload.payment_amount as any) * (FEE_AMOUNT.QR / 100)
            : FEE_AMOUNT.VA;

        admin_fee_after_vat =
          admin_fee + admin_fee * parseFloat(process.env.XENDIT_VAT_FEE);
      ...
      const xenditPaymentResult = await firstValueFrom(
        this._axiosService.onAxiosRequest(payloadXendit),
      );
      ...
      let res = await this._prismaService.payment.create({
        data: {
          ...
          payment_number:
            payload.payment_method_code == 'QRIS'
              ? xenditPaymentResult.data.qr_string
              : xenditPaymentResult.data.account_number,
          payment_date: new Date(),
          payment_status:
            payload.payment_method_code == 'QRIS'
              ? 'PENDING'
              : xenditPaymentResult.data.status,
  ...
  async simulate(id_payment: number): Promise<any> {
    try {
      const payment = await this._prismaService.payment.findUnique({
        where: {
      ...
      if (payment.payment_method != 'QRIS') {
        ...
        const getVirtualAccountXenditResult = await firstValueFrom(
          this._axiosService.onAxiosRequest(getVirtualAccountXendit),
        );
      ...
      let payloadXendit =
        payment.payment_method == 'QRIS'
          ? simulateQrCodesParams
          : simulatePaymentVirtualAccountParams;
      const xenditPaymentResult = await firstValueFrom(
        this._axiosService.onAxiosRequest(payloadXendit),
      );
      ...
      const updatePayment = await this._prismaService.payment.update({
        where: {
          id_payment: parseInt(id_payment as any),
        },
        data: {
      ...
      if (usePgAdminFee) {
        ...
        admin_fee =
          updatePayment.payment_method == 'QRIS'
            ? parseFloat(updatePayment.payment_amount as any) *
              (FEE_AMOUNT.QR / 100)
            : FEE_AMOUNT.VA;
      ...
      const updateInvoice = await this._prismaService.invoice.update({
        where: {
        ...
        data: {
          ...
          total:
            parseInt(payment.invoice.total as any) +
            parseFloat(admin_fee_after_vat as any),
        },
      });
      ...
      this._appGateway.sendPaymentNotification({
        token: updatePayment.payment_token,
      });
  ...
  async paymentCallback(payload: any): Promise<any> {
    try {
      const paymentMethod =
        payload.event && payload.event.includes('qr') ? 'QRIS' : 'VA';
      ...
      const usePgAdminFee =
        payment.invoice.pelanggan.setting_company.is_use_pg_admin_fee;
      ...
      const sendMessage = await this.sendMessage(
        {
          user: {
            id_setting_company: payment.invoice.pelanggan.id_setting_company,
          },
        } as any,
        payment.id_payment,
      );
  ...
  async editPayment(
    req: Request,
    payload: PaymentModel.UpdatePayment,
  ): Promise<any> {
    try {
      const { id_payment, ...data } = payload;

      let res = await this._prismaService.payment.update({
        where: { id_payment: parseInt(id_payment as any) },
        data: {
          ...data,
          update_at: new Date(),
  ...
  async editPaymentManual(
    req: Request,
    payload: PaymentModel.UpdatePaymentCash,
  ): Promise<any> {
    try {
      const { id_payment, ...data } = payload;

      const payment_method =
        await this._prismaService.payment_method_manual.findFirst({
          where: {
            id_payment_method_manual: parseInt(
              data.id_payment_method_manual as any,
            ),
          },
          select: {
            payment_method: true,
      ...
      let res = await this._prismaService.payment.update({
        where: { id_payment: parseInt(id_payment as any) },
        data: {
          id_invoice: parseInt(data.id_invoice as any),
  ...
  async paymentCash(
    req: Request,
    payload: PaymentModel.CreatePaymentCash,
  ): Promise<any> {
    try {
      const invoice = await this._invoiceService.getById(
        parseInt(payload.id_invoice as any),
      );
      ...
      const existingPaymentXendit = await this._prismaService.payment.findFirst(
        {
          where: {
      ...
      if (existingPaymentXendit) {
        await this._prismaService.payment.delete({
          where: {
      ...
      const paymentMethodManual =
        await this._prismaService.payment_method_manual.findFirst({
          where: {
            id_payment_method_manual: parseInt(
              payload.id_payment_method_manual as any,
            ),
          },
        });
      ...
      let resUpdateInvoice = await this._prismaService.invoice.update({
        where: {
          id_invoice: parseInt(payload.id_invoice as any),
        },
        data: {
  ...
  async cancel(req: Request, id_payment: number): Promise<any> {
    try {
      ...
      if (payment.payment_status == 'PAID') {
        const updatePaymentStatus = await this._prismaService.payment.update({
          where: {
            id_payment: parseInt(id_payment as any),
          },
          data: {
        ...
        const updateInvoiceStatus = await this._prismaService.invoice.update({
          where: {
