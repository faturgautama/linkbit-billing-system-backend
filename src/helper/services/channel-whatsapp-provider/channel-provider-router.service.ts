...
export class ChannelProviderRouterService {
  ...
  async handleSendMessage(req: Request, type: string, data: any) {
    try {
      ...
      const channel_whatsapp_default =
        await this._prismaService.setting_channel_whatsapp.findFirst({
          where: {
      ...
      const send_result =
        channel_whatsapp_default.channel_whatsapp.channel_whatsapp.includes(
          'QONTAK',
        )
          ? await this._qontakWapService.handleSendMessage(
              type,
              data,
              channel_whatsapp_default,
            )
          : await this._linkbitWapService.handleSendMessage(
              type,
              data,
              channel_whatsapp_default,
            );

      await this._prismaService.log_whatsapp_message.create({
        data: {
      ...
      return {
        ...
        message: send_result.status
          ? 'Pesan berhasil dikirimkan'
          : send_result.data.msg,
