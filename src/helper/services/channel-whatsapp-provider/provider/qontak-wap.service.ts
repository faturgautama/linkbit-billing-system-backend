...
export class QontakWapService {
    ...
    private async getListTemplate(type: string, invoice: any, channel_whatsapp: any) {
        try {
            ...
            let template_name = type == 'INVOICE'
            ...
            const payload_info = {
                ...
                message_variable: {
                    ...
                    checkout_url: `${process.env.CHECKOUT_URL}?token=${this._utilityService.onEncrypt(JSON.stringify(invoice.id_invoice))}`,
                    invoice_digital_url: `${process.env.INVOICE_DIGITAL_URL}?token=${this._utilityService.onEncrypt(JSON.stringify(invoice.id_invoice))}`,
                },
            }
